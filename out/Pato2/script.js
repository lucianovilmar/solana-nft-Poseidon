...
// --- SISTEMA DE GERAÇÃO DE PATOS ---

const duckAssets = {
    colors: ['#facc15', '#fde047', '#fbbf24', '#84cc16', '#22c55e', '#06b6d4', '#3b82f6', '#8b5cf6', '#d946ef', '#f43f5e'],
    
    patterns: [
        { id: 'none', svg: '' },
        { id: 'dots', svg: `<pattern id="patDots" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse"><circle cx="5" cy="5" r="2" fill="rgba(0,0,0,0.2)"/></pattern>` },
        { id: 'stripes', svg: `<pattern id="patStripes" x="0" y="0" width="10" height="10" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><rect width="5" height="10" fill="rgba(0,0,0,0.15)"/></pattern>` },
        { id: 'zebra', svg: `<pattern id="patZebra" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M0 0 Q 5 10 10 5 Q 15 0 20 10 L 20 15 Q 15 5 10 10 Q 5 15 0 5 Z" fill="rgba(0,0,0,0.7)"/></pattern>` }
    ],
...
function generateElaborateDuckSVG() {
    const baseColor = duckAssets.colors[Math.floor(Math.random() * duckAssets.colors.length)];
    const patternData = duckAssets.patterns[Math.floor(Math.random() * duckAssets.patterns.length)];
    const hat = duckAssets.hats[Math.floor(Math.random() * duckAssets.hats.length)];
    const accessory = duckAssets.accessories[Math.floor(Math.random() * duckAssets.accessories.length)];
    
    const uniqueId = Math.random().toString(36).substr(2, 9);
    const patternFill = patternData.id !== 'none' ? `url(#${patternData.id})` : 'none';

    const duckBodyPath = "M80 70 C80 45 60 40 40 40 C15 40 5 55 5 70 C5 85 20 90 45 90 C70 90 80 85 80 70 Z";
    const duckHeadPath = "M62 17 A 18 18 0 1 1 62 53 A 18 18 0 1 1 62 17 Z";
    ...
    <svg width="90" height="100" viewBox="0 -10 100 110">
        <defs>
            ${patternData.svg}
            <mask id="duckMask_${uniqueId}">
                <path d="${duckBodyPath}" fill="white"/>
                <path d="${duckHeadPath}" fill="white"/>
            </mask>
        </defs>

        <g stroke="black" stroke-width="2.5">
            <path d="${duckBodyPath}" fill="${baseColor}"/>
            <path d="${duckHeadPath}" fill="${baseColor}"/>
        </g>

        <g mask="url(#duckMask_${uniqueId})">
            <rect x="0" y="0" width="100" height="100" fill="${patternFill}" opacity="0.6"/>
        </g>

        <g>
            <circle cx="65" cy="30" r="4" fill="white" stroke="black" stroke-width="1.5"/>
            <circle cx="66" cy="29" r="1.5" fill="black"/>
        </g>
